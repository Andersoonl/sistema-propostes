...
import { fmtReceivableNumber, fmtOrderNumber, fmtMoney } from '@/lib/format'
import {
  ...
  type ReceivablePaginationParams,
  type ReceivableKPIs,
...
interface ReceberClientProps {
  initialData: PaginatedData
  initialKPIs: ReceivableKPIs
  customers: CustomerOption[]
  readyOrders: ReadyOrder[]
}

export function ReceberClient({ initialData, initialKPIs, customers: initialCustomers, readyOrders: initialReadyOrders }: ReceberClientProps) {
  const [receivables, setReceivables] = useState<ReceivableRow[]>(initialData.data)
  const [total, setTotal] = useState(initialData.total)
  const [page, setPage] = useState(initialData.page)
  const [totalPages, setTotalPages] = useState(initialData.totalPages)
  const [kpis, setKpis] = useState<ReceivableKPIs>(initialKPIs)
  ...
  const [payReceivable, setPayReceivable] = useState<ReceivableRow | null>(null)
  ...
  const fetchData = useCallback(async (targetPage: number) => {
    setLoading(true)
    try {
      const params: ReceivablePaginationParams = {
        page: targetPage,
        pageSize: PAGE_SIZE,
      ...
      const result = await getReceivablesPaginated(params)
  ...
  const refreshAll = async () => {
    const [, newKpis, newCustomers, newOrders] = await Promise.all([
      fetchData(page),
      getReceivableKPIs(),
      getActiveCustomers(),
      getReadyOrdersForReceivable(),
  ...
  const handleOpenEdit = (r: ReceivableRow) => {
    setEditId(r.id)
    setFormOrderId(r.orderId || '')
    setFormCustomerId(r.customerId)
    setFormDescription(r.description || '')
    setFormInvoice(r.invoiceNumber || '')
    setFormIssueDate(r.issueDate.split('T')[0])
    setFormDueDate(r.dueDate.split('T')[0])
    setFormAmount(String(r.totalAmount))
  ...
  const handleSelectOrder = (orderId: string) => {
    ...
    if (orderId) {
      ...
      if (order) {
        setFormCustomerId(order.customer.id)
  ...
  const handleSaveReceivable = async () => {
    ...
    try {
      ...
      if (editId) {
        await updateReceivable(editId, data)
      } else {
  ...
  const handleOpenPayment = async (r: ReceivableRow) => {
    setPayReceivable(r)
    setPayAmount(String(Math.round((r.totalAmount - r.paidAmount) * 100) / 100))
    setPayDate(todayStr())
    setPayMethod('PIX')
    setPayReference('')
    setPayNotes('')
    ...
    try {
      const payments = await getReceivablePayments(r.id)
  ...
  const handleSavePayment = async () => {
    if (!payReceivable) return
    setPaySaving(true)
    setPayError(null)
    try {
      await addReceivablePayment(payReceivable.id, {
  ...
  const handleDeletePayment = async (paymentId: string) => {
    ...
    try {
      ...
      if (payReceivable) {
        const payments = await getReceivablePayments(payReceivable.id)
        setPayPayments(payments)
      }
      await refreshAll()
      // Atualizar payReceivable com dados frescos
      setShowPayModal(false)
    } catch (err) {
  ...
  const handleDelete = async (r: ReceivableRow) => {
    if (!confirm(`Excluir conta ${fmtReceivableNumber(r.number)}?`)) return
    try {
  ...
  const handleCancel = async (r: ReceivableRow) => {
    if (!confirm(`Cancelar conta ${fmtReceivableNumber(r.number)}?`)) return
    try {
      await cancelReceivable(r.id)
      await refreshAll()
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {receivables.length === 0 && !loading ? (
          <div className="p-8 text-center text-gray-500">
            {total === 0 && statusFilter === 'ALL' && !search
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:text-gray-700 select-none" onClick={() => handleSort('customer')}>
                  ...
                  {receivables.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap font-medium text-[#2d3e7e]">
                        {fmtReceivableNumber(r.number)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-gray-900">
                        {r.customer.tradeName || r.customer.companyName}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-[#3bbfb5] font-medium">
                        {r.order ? fmtOrderNumber(r.order.number) : '-'}
                      </td>
                      ...
                      <td className={`px-4 py-3 whitespace-nowrap font-medium ${r.status === 'OVERDUE' ? 'text-red-600' : 'text-gray-600'}`}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-right">
                        <span className={r.paidAmount > 0 ? 'text-green-600 font-medium' : 'text-gray-400'}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-center">
                        <div className="flex items-center justify-center gap-1 flex-wrap">
                          ...
                          {r.status !== 'PAID' && r.status !== 'CANCELLED' && r._count.payments === 0 && (
                            <button
      ...
      {/* Modal Criar/Editar Conta a Receber */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">
                {editId ? 'Editar Conta a Receber' : 'Nova Conta a Receber'}
              </h2>
              ...
              {/* Pedido (só na criação) */}
              {!editId && (
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Pedido Entregue (opcional)</label>
                  <select
                    value={formOrderId}
                  ...
                  >
                    ...
                    {readyOrders.map((o) => (
                      <option key={o.id} value={o.id}>
                        {fmtOrderNumber(o.number)} — {o.customer.tradeName || o.customer.companyName} — {fmtMoney(o.totalAmount)}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="Ex: Venda de pisos para obra X"
                />
              </div>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  ...
                  disabled={formSaving || !formCustomerId || !formDueDate || !formAmount}
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium text-[#2d3e7e]">{fmtReceivableNumber(payReceivable.number)}</div>
                  </div>
                  <div>
                    <span className="text-gray-500">Cliente</span>
                    <div className="font-medium">{payReceivable.customer.tradeName || payReceivable.customer.companyName}</div>
                  </div>
                  <div>
                    <span className="text-gray-500">Valor Total</span>
                    <div className="font-medium">{fmtMoney(payReceivable.totalAmount)}</div>
                  </div>
                  <div>
                    <span className="text-gray-500">Restante</span>
                    <div className="font-bold text-red-600">
                      {fmtMoney(payReceivable.totalAmount - payReceivable.paidAmount)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    min="0.01"
                    max={payReceivable.totalAmount - payReceivable.paidAmount}
                    value={payAmount}
